...
- Do we store only validated events, or store invalid events in a quarantine table for diagnostics?
  - **TODO:** Persist invalid kind `33301` events into an additional table (quarantine/diagnostics) so we can inspect validation failures (including `openprints_schema` + lineage rules), fix event payloads, and optionally re-broadcast once rules are corrected.
