...
def test_load_indexer_config_none_when_no_file(monkeypatch, tmp_path) -> None:
    ...
    config, errors, path = load_indexer_config(None)
    assert config == {}
    assert errors == []
    assert path is None
...
def test_load_indexer_config_loads_index_section(tmp_path) -> None:
    ...
    config, errors, path = load_indexer_config(str(config_file))
    assert errors == []
...
def test_load_indexer_config_relative_path(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.chdir(tmp_path)
    config, errors, path = load_indexer_config(DEFAULT_CONFIG_FILENAME)
    assert errors == []
...
def test_load_indexer_config_file_not_found() -> None:
    config, errors, path = load_indexer_config("/nonexistent/path.toml")
    assert config == {}
    assert len(errors) == 1
...
def test_load_indexer_config_os_error_when_path_is_directory(monkeypatch, tmp_path) -> None:
    ...
    config, errors, path = load_indexer_config(str(tmp_path))
    assert config == {}
    assert len(errors) == 1
